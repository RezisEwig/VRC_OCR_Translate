...
[한국어](#korean) · [English](#english) · [日本語](#japanese) · [繁體中文](#traditional-chinese)
...
**這是一款本機 AI 工具，可讀取 VRChat 中的文字，並在接近原文的位置顯示翻譯字幕。**

不需要 API 金鑰或外部翻譯服務，OCR 與翻譯都在您的電腦上執行。✨

</div>

## 🎬 示範影片

[![自動翻譯、手動翻譯與字幕位置調整示範](assets/demo-preview-en.gif)](assets/demo-en.mp4)

GIF 可直接在 GitHub 上播放，點擊後可開啟完整的 1080p 示範影片。影片左上角的功能說明使用英文。

## 🌟 主要功能

- 自動辨識韓文、日文、中文、英文與主要歐洲語言。
- 翻譯成您在 **我的語言** 中選擇的語言。
- 控制面板會一併切換成所選語言。
- 可自動偵測原文語言，也能指定單一原文語言。
- 將每段翻譯字幕放在接近原文畫面位置的地方。
- 字幕互相重疊時，會移動到附近的空白位置。
- 支援每 2 秒執行一次的自動翻譯，以及需要時才啟動的手動翻譯。
- 只擷取 VRChat 遊戲視窗，避免翻譯到桌面內容或再次辨識已顯示的字幕。
- 使用本機 RapidOCR 與 TranslateGemma，不依賴 Papago 或 DeepL。🔒

## 🚀 簡易安裝

不需要會使用 PowerShell，也不必手動安裝 Python。

1. 點擊 GitHub 頁面上方的 **Code → Download ZIP**。
2. 解壓縮下載的 ZIP 檔案。
3. 連按兩下 **`INSTALL.bat`**。
4. 安裝完成後啟動 Virtual Desktop、SteamVR 與 VRChat。
5. 連按兩下 **`RUN_TRANSLATOR.bat`**。

第一次安裝會下載約 3 GB 的資料。安裝程式會自動準備 Python 3.12、必要套件、OCR 模型、TranslateGemma 與 llama.cpp Vulkan 執行環境。

## 🎛️ 控制面板

| UI 項目 | 功能 |
| --- | --- |
| **我的語言** | 翻譯字幕與控制面板所使用的目標語言 |
| **要翻譯的語言** | VRChat 畫面中的原文語言，可選擇自動偵測或指定語言 |
| **自動翻譯 / 手動翻譯** | 每 2 秒自動翻譯，或僅在要求時翻譯 |
| **立即翻譯** | 翻譯目前畫面一次，並保留顯示結果 |
| **清除字幕** | 清除 VR 中所有翻譯字幕 |
| **字幕位置** | 移動、縮放或重設字幕疊加位置 |

<p align="center">
  <img src="assets/control-panel-zh-tw.png" alt="繁體中文控制面板" width="360">
</p>

畫面上可能同時出現多種語言時，請使用 **自動偵測**。如果已知原文語言，建議直接指定該語言。這樣可以略過語言判斷，只執行適合的 OCR 辨識器，通常能降低 CPU 負擔、縮短處理時間，並提升翻譯品質。

例如，將 **我的語言** 設為韓文、**要翻譯的語言** 設為 `日本語`，程式會使用日文專用 OCR 與日文翻譯成韓文的專用提示。對只使用日文的世界來說，通常比自動偵測更準確、穩定。
...
語言選項、字幕校正值，以及最後使用的自動／手動翻譯模式都會儲存在 `config.json`。

## 🎮 操作方式

| 輸入 | 動作 |
| --- | --- |
| 左手控制器扳機鍵 | 翻譯目前畫面一次，並保留字幕 |
| 左手控制器握把鍵 | 清除所有翻譯字幕 |
| `Ctrl+Alt+T` | 切換自動翻譯與手動翻譯 |
| `Ctrl+Alt+左/右` | 將全部字幕向左或向右移動 |
| `Ctrl+Alt+上/下` | 將全部字幕向上或向下移動 |
| `Ctrl+Alt+數字鍵盤 + / -` | 調整字幕位置的縮放比例 |
| `Ctrl+Alt+Home` | 重設字幕位置與縮放比例 |

程式只會監聽控制器輸入而不攔截它，因此扳機鍵與握把鍵在 VRChat 中原有的功能仍會正常運作。

## 🖥️ 測試環境與建議規格

| 項目 | 詳細資訊 |
| --- | --- |
| 已測試頭戴裝置 | Meta Quest Pro |
| PC 連線方式 | Virtual Desktop |
| VR 執行環境 | SteamVR / OpenVR |
| 已測試 GPU | NVIDIA GeForce RTX 5070 Ti |
| 作業系統 | Windows 11 |
| 記憶體 | 建議 16 GB 以上 |
| VRAM | 建議 8 GB 以上 |
| 可用儲存空間 | 約 4 GB |

RapidOCR 使用 CPU，TranslateGemma 則透過 Vulkan 使用 GPU。翻譯模型本身約占用 3 GB VRAM，因此與 VRChat 同時執行時，建議保留足夠的 GPU 餘裕。

## ⚠️ 限制

- 目前只在 Quest Pro + Virtual Desktop + SteamVR 的組合上進行過實機測試。
- 其他 Quest 裝置、Steam Link 與有線 PC VR 可能可以運作，但尚未驗證。
- 字幕位置以可見的 2D 畫面為基準，不會貼合實際的 3D 物件表面。
- 直排文字、裝飾字型、非常小或模糊的文字可能無法辨識。
- 只包含共用漢字的日文與中文，有時難以透過自動偵測正確區分。
- 本機 4B 模型可能誤譯專業術語、專有名詞與複雜長文。
- 尚未實作 Quest Pro 眼動追蹤翻譯。
- 本專案並非 VRChat、SteamVR、Virtual Desktop 或 Meta 的官方專案。

## 🧩 運作方式

```text
VRChat 遊戲視窗
  → 日文專用 / 東亞語言 / 韓文 / 拉丁字母 OCR
  → TranslateGemma 翻譯成「我的語言」
  → 透過 SteamVR 疊加層顯示定位字幕
```

實作細節請參閱 [DESIGN.md](DESIGN.md)。

## 📜 授權與製作方式

程式碼與文件採用 [MIT License](LICENSE)。TranslateGemma 另受 [Gemma Terms of Use](https://ai.google.dev/gemma/terms) 規範，其他相依套件列於 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。

**本儲存庫中的程式碼、指令碼、測試與文件皆由 OpenAI Codex 完成。** RezisEwig 提供構想、需求、Quest Pro 實機測試與反覆回饋。
